...
impl AsyncFsmSender {
    pub fn new(api_key: String, fcm_url: String) -> AsyncFsmSender {
        ...
        headers.insert(AUTHORIZATION, ("key=".to_string() + &api_key).parse().unwrap());
        headers.insert(CONTENT_TYPE, "application/json".parse().unwrap());


        let client = ClientBuilder::new().default_headers(headers).build().expect("new async client");
