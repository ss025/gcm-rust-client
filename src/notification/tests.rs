...
fn should_set_notification_body() {
    ...
    let nm = NotificationBuilder::new("title").body("body").finalize();
    ...
    assert_eq!(
        json_result.unwrap(),
        r#"{"title":"title","body":"body","icon":"myicon"}"#
    );
}
...
fn should_set_notification_icon() {
    let nm = NotificationBuilder::new("title").icon("newicon").finalize();
...
fn should_set_notification_badge() {
    ...
    let nm = NotificationBuilder::new("title").badge("1").finalize();
...
fn should_set_notification_tag() {
    ...
    let nm = NotificationBuilder::new("title").tag("tag").finalize();
...
fn should_set_notification_color() {
    ...
    let nm = NotificationBuilder::new("title").color("color").finalize();
...
fn should_set_notification_body_loc_args() {
    ...
    assert_eq!(
        json_result.unwrap(),
        r#"{"title":"title","icon":"myicon","body_loc_args":["args"]}"#
    );
}
...
fn should_set_notification_title_loc_args() {
    ...
    assert_eq!(
        json_result.unwrap(),
        r#"{"title":"title","icon":"myicon","title_loc_args":["args"]}"#
    );
}
